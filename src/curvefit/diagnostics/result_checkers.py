...
class LogDerfLinearRegressionChecker(ResultChecker):
    ...
    def __init__(self, df, col_log_derf_obs, col_est, est_space, col_group, col_t):
    ...
    def check_result(self, groups=None): 
        ...
        for grp, df in df_by_group.items():
            ...
            lr_log_derf.fit()
            metric_fun = lambda est: np.linalg.norm(est - log_derf_obs)
            log_derf_rmses.update(lr_log_derf.compare([estimates], [grp], metric_fun))
        
        result_df = pd.from_dict(log_derf_rmses, orient='index', columns=['curr model', 'baseline'])
        return result_df
